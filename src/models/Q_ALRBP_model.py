...
"""
Quadratic ALRBP model.

This module implements the quadratic formulation of the Assembly Line Rebalancing Problem (ALRBP)
with workload fairness, ergonomic fairness, and similarity preservation objectives.
The workstation–area compatibility constraint is expressed through bilinear terms involving
task-to-station assignment variables, requiring a non-convex mixed-integer quadratic formulation.
"""


def build_q_alrbp_model(
    ...
    norm_values,                 # dict: f1_min/max, f2_min/max, f3_min/max
    weights=(1/3, 1/3, 1/3),     # (alpha,beta,gamma) come nel tuo script
):
    """
    Builds the quadratic ALRBP Gurobi model (area constraint quadratic via bilinear terms).
    ...
    # ---------------- CONSTRAINTS ----------------
    # each task assigned once to worker and station
    for i in tasks:
    ...
    # consistency x<=y or not in station
    for i in tasks:
        for s in stations:
            for w in workers:
    ...
    # precedence: forbid i in later station than j
    for j in tasks:
    ...
    # QUADRATIC area mismatch constraint (your “quadratic formulation” feature)
    for w in workers:
        m.addConstr(
    ...
    # ---- MSF (linearized with auxiliary w[i,j,s]) as in your code ----
    w_aux = {}
    SF_exprs = []
    for i in tasks:
        TIB_i = TIB.get(i, [])
    ...
    f1_expr = gp.LinExpr(w_balance)        # workload balance (min)
    f2_expr = gp.LinExpr(erg_balance)      # ergonomic balance (min)
    f3_expr = gp.LinExpr(MSF_expr)         # MSF (max)
    ...
    eps = 1e-6
    f1_norm = (f1_expr - f1_min) / max(f1_max - f1_min, eps)        # min
    f2_norm = (f2_expr - f2_min) / max(f2_max - f2_min, eps)        # min
    f3_norm = (f3_max - f3_expr) / max(f3_max - f3_min, eps)        # convert max->min
